<template>
  <div class="newsTicker w-100 d-flex align-items-center">
    <div class="newsTicker-text d-flex align-items-center" v-if="announment">
      <i class="bi bi-exclamation-triangle-fill"></i>{{ announment.message }}
    </div>
  </div>
</template>
...
onMounted(() => {
  width.value = document.querySelector(".newsTicker")?.getBoundingClientRect()
    .width as number;
  announmentWidth.value = document.querySelector(".newsTicker-text")
  ...
  if (width.value) {
    gsap.fromTo(
      ...
      { x: width.value },
      {
        ...
        repeat: runTimes.value,
        x: -announmentWidth.value,
        ease: Power0.easeIn,
      }
    );
  }
...
function stopTickerAnimate() {
  if (width.value) {
    gsap.fromTo(".newsTicker-text", { opacity: 0 }, {});
  }
}
