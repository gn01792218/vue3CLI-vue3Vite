<template>
  ...
  <div class="secRoad d-flex sideWidth">
    <div
      class="secRoad-column"
      :class="`secRoad-column${index}`"
      v-for="(sc, index) in secWidth"
      :key="index"
    ></div>
  </div>
  <div class="sec-column position-absolute sideWidth">
    <div
      class="secRoad-row"
      :class="[{ 'boundary-bottom': (index === 5) | (index === 8) }]"
      v-for="(sr, index) in secHeight"
      :key="index"
    ></div>
  </div>
  <!-- bigRoadGrid -->
  <section class="bigRoad d-flex sideWidth position-absolute">
    <div
      class="bigRoad-column"
      :class="[`bigRoad-column${index}`]"
      v-for="(tc, index) in secWidth"
      :key="index"
    >
      <div
        class="bigRoad-item d-flex"
        :class="[`bigRoad-item${index}`]"
        v-for="(tci, index) in topHeight"
        :key="index"
      >
        <div>1</div>
      </div>
  ...
  setup() {
    ...
    watch(askRoadArr.value, () => {
      ...
      let column = document.querySelector(
        `.bigRoad-column${bigRoadColumn.value}`
      ) as HTMLElement;
      let road: HTMLElement;
      if (bigRoadItemIndex.value > 0) {
        road = column.children[bigRoadItemIndex.value - 1]
          .firstChild as HTMLElement;
      } else {
        road = column.children[bigRoadItemIndex.value]
          .firstChild as HTMLElement;
      }
    ...
    function askRoad(roadNum: number) {
      ...
      //換行一:不同陣營
      if (
        currentBigRoadResult.value !== lastBigRoadResult.value &&
        ...
        if (roadOverFlowerTimes.value != 0) {
          ...
          if (bigRoadItemIndex.value - 1 < 1) {
            ...
            bigRoadColumn.value =
              bigRoadColumn.value - roadOverFlowerTimes.value + 1;
            roadOverFlowerTimes.value = 0;
          }
        ...
        if (
          bigRoadColumn.value >=
          secWidth.length + (bigRoadColArr.length - secWidth.length)
        ) {
          //溢出極限格子的時候要增加行數
          // console.log("滿了+行")
          addBigRoadColumn();
      ...
      //溢出時如果遇到和局，其實不需要+行?!
      if (
        bigRoadColArr[bigRoadColumn.value][bigRoadItemIndex.value] !== 0 ||
        ...
        for (let i = bigRoadItemIndex.value; i < 6; i++) {
    ...
    function putBigRoad(gameResult: number, tieCount: number) {
      ...
      let bigRoadCol = document.querySelector(
        `.bigRoad-column${bigRoadColumn.value}`
      ) as HTMLElement;
      let bigRoadColItem = bigRoadCol.children[bigRoadItemIndex.value]
        .firstChild as HTMLElement;
      switch (gameResult) {
    ...
    function resetBigRoad() {
      ...
      let bigRoadColContainer = document.querySelector(
        ".bigRoad"
      ) as HTMLElement;
