<template>
  <div class="smallRoad d-flex w-50">
    <div
      class="smallRoad-column"
      :class="[`smallRoad-column${index}`, { 'boundary-right': index === 21 }]"
      v-for="(b1, index) in bottom1width"
      :key="index"
    >
      <div
        class="smallRoad-item"
  ...
  setup() {
    ...
    watch(askRoadRecall, () => {
      ...
      let column = document.querySelector(
        `.smallRoad-column${smallRoadColumn.value}`
      ) as HTMLElement;
      let road: HTMLElement;
      if (smallRoadItemIndex.value > 0) {
        road = column.children[smallRoadItemIndex.value - 1]
          .firstChild as HTMLElement;
      } else {
        road = column.children[smallRoadItemIndex.value]
          .firstChild as HTMLElement;
      }
    ...
    function askRoad(roadNum: number) {
      recordRoad(roadNum);
      if (
        currentsmallRoadResult.value !== lastsmallRoadResult.value &&
        ...
        if (roadOverFlowerTimes.value != 0) {
          //第一次恢復的時候
          if (smallRoadItemIndex.value - 1 < 1) {
            //因為上一次已經被+過了，要減回來
            ...
            smallRoadColumn.value =
              smallRoadColumn.value - roadOverFlowerTimes.value + 1;
            roadOverFlowerTimes.value = 0;
          }
        ...
        if (
          smallRoadColumn.value >=
          bottom1width.length + (smallRoadColArr.length - bottom1width.length)
        ) {
          //溢出極限格子的時候要增加行數
          // console.log("滿了+行")
          addSmallRoadColumn();
      ...
      //溢出時如果遇到和局，其實不需要+行?!
      if (
        smallRoadColArr[smallRoadColumn.value][smallRoadItemIndex.value] !==
          0 ||
        smallRoadItemIndex.value > 5
      ) {
        // console.log("連贏溢出")
        ...
        for (let i = smallRoadItemIndex.value; i < 6; i++) {
    ...
    function putRoad(gameResult: number) {
      ...
      //  console.log(RoadCol)
      let bigRoadColItem = RoadCol.children[smallRoadItemIndex.value]
        .firstChild as HTMLElement;
      switch (gameResult) {
        case 1:
      ...
      lastSmallRoadDataLength.value =
        smallRoadResult.value.columns[
          smallRoadResult.value.columns.length - 1
        ].blocks.length;
    ...
    function addSmallRoadColumn() {
      ...
      let preRoad = document.querySelector(
        `.smallRoad-column${smallRoadColumn.value - 1}`
      ) as HTMLElement;
        ...
        item.blocks.forEach((i: any) => {
          ...
          if (
            ...
            if (
              smallRoadColumn.value >=
              bottom1width.length +
                (smallRoadColArr.length - bottom1width.length)
            ) {
              //溢出極限格子的時候要增加行數
              // console.log("滿了+行")
              addSmallRoadColumn();
