  ...
  setup() {
    ...
    watch(askRoadArr.value, () => {
      ...
      let column = document.querySelector(
        `.beadPlate-column${beadPlateColumnCount.value}`
      ) as HTMLElement;
    ...
    function putRoad(columnNum: number, roadnum: number, gameResult: number) {
      let beadPlateCol = document.querySelector(
    ...
    function askRoad(road: number) {
      if (
        beadPlateColumnCount.value >= beadPlateColumn.length - 1 &&
        roadIndex.value > beadPlateRow.length - 1
      ) {
        addColumn();
      }
      if (roadIndex.value > beadPlateRow.length - 1) {
        //row放滿時
    ...
    function addColumn() {
      ...
      beadPlateColumnCount.value++;
      roadIndex.value = 0;
      let beadPlate = document.querySelector(
        ".beadPlatRoadPlace"
      ) as HTMLElement;
    ...
    function resetRoad() {
      ...
      let beadPlatePlace = document.querySelector(
        ".beadPlatRoadPlace"
      ) as HTMLElement;
