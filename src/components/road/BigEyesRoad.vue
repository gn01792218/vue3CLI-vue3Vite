<template>
  <section class="BigEyesRoad d-flex sideWidth position-absolute">
    <div
      class="BigEyesRoad-colum"
      :class="[`BigEyesRoad-colum${index}`]"
      v-for="(cc, index) in BigEyesRoadWidth"
      :key="index"
    >
      <div
        class="BigEyesRoad-item"
        :class="[`BigEyesRoad-item${index}`]"
        v-for="(cci, index) in bottomHeight"
        :key="index"
      >
        <div></div>
      </div>
  ...
  setup() {
    ...
    watch(askRoadRecall, () => {
      ...
      let column = document.querySelector(
        `.BigEyesRoad-colum${bigEyesRoadColumn.value}`
      ) as HTMLElement;
      let road: HTMLElement;
      if (bigEyesRoadItemIndex.value > 0) {
        road = column.children[bigEyesRoadItemIndex.value - 1]
          .firstChild as HTMLElement;
      } else {
        road = column.children[bigEyesRoadItemIndex.value]
          .firstChild as HTMLElement;
      }
    ...
    function putBigEyesRoad(gameResult: number) {
      ...
      let RoadColItem = RoadCol.children[bigEyesRoadItemIndex.value]
        .firstChild as HTMLElement;
      switch (gameResult) {
        case 1:
      ...
      bigEyesRoadColArr[bigEyesRoadColumn.value][
        bigEyesRoadItemIndex.value
      ] = 1; //代表那一格已經畫過了
      bigEyesRoadItemIndex.value++; //增加當前的index
      lastbigEyesRoadResult.value = currentbigEyesRoadResult.value; //將這次陣營記錄到下一次的陣營中
      lastbigEyesRoadDataLength.value =
        bigEyesRoadResult.value.columns[
          bigEyesRoadResult.value.columns.length - 1
        ].blocks.length; //將這次的最後一個columns的blocks的長度紀錄起來
      lastBigEyesRoadColumnLength.value =
        bigEyesRoadResult.value.columns.length;
    ...
    function askRoad(roadNum: number) {
      ...
      recordRoad(roadNum);
      if (
        currentbigEyesRoadResult.value !== lastbigEyesRoadResult.value &&
        ...
        if (roadOverFlowerTimes.value != 0) {
          //第一次恢復的時候
          if (bigEyesRoadItemIndex.value - 1 < 1) {
            //因為上一次已經被+過了，要減回來
            ...
            bigEyesRoadColumn.value =
              bigEyesRoadColumn.value - roadOverFlowerTimes.value + 1;
            roadOverFlowerTimes.value = 0;
          }
        ...
        if (
          bigEyesRoadColumn.value >=
          BigEyesRoadWidth.length +
            (bigEyesRoadColArr.length - BigEyesRoadWidth.length)
        ) {
          //溢出極限格子的時候要增加行數
          // console.log("滿了+行")
      ...
      if (
        ...
        for (let i = bigEyesRoadItemIndex.value; i < 6; i++) {
